...
### Disabling un-used Interfaces
Most commonly, access to Raspberry Pi is through Ethernet. Since Raspberry Pi has many interfaces, it's better to turn them off.
...
```
### Searching and purging packages
...
So, the steps to generate key based authentication are given below:
...
```
If ssh-copy-id command is not available, you can manually copy the file over ssh.
```
...
Check for errors under /var/log/secure on Raspberry Pi.
